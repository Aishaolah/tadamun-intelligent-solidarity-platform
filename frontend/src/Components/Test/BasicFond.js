...
export default function BasicFond() {
  const [posts, setPosts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [showAll, setShowAll] = useState(false);

  useEffect(() => {
    const fetchBasicNeedsPosts = async () => {
      try {
        setLoading(true);

        const res = await fetch(
          "http://localhost:3001/api/posts?category=basic_needs"
        );
    ...
    return (
      <div className="ml-4 md:ml-20 mr-4 md:mr-20 mt-16">
        <h1 className="text-xl md:text-2xl font-bold">
          Browse basic needs fundraisers
        </h1>
      </div>
    );
  }
    ...
    return (
      <div className="ml-4 md:ml-20 mr-4 md:mr-20 mt-16">
        ...
        <p className="mt-6 text-gray-500 text-center">
          No basic needs fundraisers available yet.
        </p>
      </div>
    );
  }
